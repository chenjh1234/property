...
//void  initPropertyBrowserTranslation(QCoreApplication* app, char const* lang)
void initPropertyBrowserTranslation(QCoreApplication* app, char const* lang)
{
    if (strcmp(lang, "zh") == 0) {
        QScopedPointer<QTranslator> translator(new QTranslator);
        if (translator->load(":/current/translations/qtpropertybrowser_zh.qm")) {
...
{
    return QString();
//    return QApplication::translate("QtPropertyBrowserUtils", "[%1, %2, %3]", 0, )
//                                  .arg(QString::number(c.red()))
//                                  .arg(QString::number(c.green()))
//                                  .arg(QString::number(c.blue()));
//                                  .arg(QString::number(c.alpha())) (%4)
}
...
{
    return QApplication::translate("QtPropertyBrowserUtils", "[%1, %2]", 0)// QCoreApplication::UnicodeUTF8)
                                  .arg(f.family())
                                  .arg(f.pointSize());
}
