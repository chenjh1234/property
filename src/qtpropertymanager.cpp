...
public:
    ...
    struct Data
    {
        Data() : invalidValue(-1), val(invalidValue)  {}
        int invalidValue;
        int val;
        QStringList enumNames;
        QMap<int, QIcon> enumIcons;
        QMap<int, int> indexToValue;
        QMap<int, int> valueToIndex;

        int getValueFromIndex(int index) const;
        int getIndexFromValue(int value) const;
    };
...
int QtEnumPropertyManagerPrivate::Data::getValueFromIndex(int index) const
{
    return indexToValue.isEmpty() ? index : indexToValue.value(index, invalidValue);
}

int QtEnumPropertyManagerPrivate::Data::getIndexFromValue(int value) const
{
    return valueToIndex.isEmpty() ? value : valueToIndex.value(value, -1);
}
...
{
    QMap<int, int> indexToValue = getData<QMap<int, int> >(d_ptr->m_values,
                                                           &QtEnumPropertyManagerPrivate::Data::indexToValue,
                                                           property,
                                                           QMap<int, int>());
...
QString QtEnumPropertyManager::valueText(const QtProperty *property) const
{
    ...
    const int val = data.val;
    const int idx = data.getIndexFromValue(val);
    if (idx >= 0 && idx < data.enumNames.count())
        return data.enumNames.at(idx);
    return QString();
}
...
void QtEnumPropertyManager::setValue(QtProperty *property, int val)
{
    ...
    if (idx >= data.enumNames.count())
        return;

    if (idx < 0 && data.enumNames.count() > 0)
        return;

    if (idx < 0)
        val = data.invalidValue;
...
void QtEnumPropertyManager::setEnumNames(QtProperty *property, const QStringList &enumNames)
{
    ...
    data.val = data.invalidValue;

    if (enumNames.count() > 0)
        data.val = data.getValueFromIndex(0);
...
void QtEnumPropertyManager::setEnumValues(QtProperty *property, const QList<int> &values)
{
    const QtEnumPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;
    ...
    if (data.indexToValue.values() == values) // ??? Work or not, unordered set ? ordered set ?
        return;

    int prevIndex = (data.valueToIndex.isEmpty() ? data.val : data.getIndexFromValue(data.val));
    data.indexToValue.clear();
    data.valueToIndex.clear();

    if (values.isEmpty()) {
        data.invalidValue = -1;
        if (prevIndex >= 0 && prevIndex < data.enumNames.count())
            data.val = prevIndex;
        else
            data.val = -1;
    }
        ...
        for (int idx = 0; idx < values.count(); idx++) {
            data.indexToValue.insert(idx, values[idx]);
            data.valueToIndex.insert(values[idx], idx);
        }
        data.invalidValue = (data.valueToIndex.constBegin().key() - 1);
        data.val = data.getValueFromIndex(prevIndex);
    }
...
int QtEnumPropertyManager::indexFromValue(QtProperty *property, int value) const
{
    const QtEnumPropertyManagerPrivate::PropertyValueMap::const_iterator it = d_ptr->m_values.constFind(property);
    if (it == d_ptr->m_values.constEnd())
        return -1;
    ...
    QtEnumPropertyManagerPrivate::Data data = it.value();
    return data.getIndexFromValue(value);
}

int QtEnumPropertyManager::valueFromIndex(QtProperty *property, int index) const
{
    const QtEnumPropertyManagerPrivate::PropertyValueMap::const_iterator it = d_ptr->m_values.constFind(property);
    if (it == d_ptr->m_values.constEnd())
        return index;
    ...
    QtEnumPropertyManagerPrivate::Data data = it.value();
    return data.getValueFromIndex(index);
}

int QtEnumPropertyManager::index(const QtProperty *property) const
{
    const QtEnumPropertyManagerPrivate::PropertyValueMap::const_iterator it = d_ptr->m_values.constFind(property);
    if (it == d_ptr->m_values.constEnd())
        return -1;
    ...
    QtEnumPropertyManagerPrivate::Data data = it.value();
    return data.getIndexFromValue(data.val);
}
...
public:
    ...
    void slotDeliverOpenPath(QtProperty* property);
};

void QtPathPropertyManagerPrivate::slotDeliverOpenPath(QtProperty* property)
{
    emit q_ptr->openPathTriggered(property, q_ptr);
}
...
class QtCustomPropertyManagerPrivate
{
public:
    QtCustomPropertyManager *q_ptr;
    Q_DECLARE_PUBLIC(QtCustomPropertyManager)
public:
    struct Data
    {
        QVariant val;
    ...
    void slotDeliverOpenEvent(QtProperty* property);
    void slotDeliverPopupEvent(QtProperty* property);
};

void QtCustomPropertyManagerPrivate::slotDeliverOpenEvent(QtProperty *property)
{
    emit q_ptr->customOpenTriggered(property, q_ptr);
}

void QtCustomPropertyManagerPrivate::slotDeliverPopupEvent(QtProperty *property)
{
    emit q_ptr->customPopupTriggered(property, q_ptr);
}

/*!
 * implementation of QtCustomPropertyManager
 */

QtCustomPropertyManager::QtCustomPropertyManager(QObject *parent)
    : QtAbstractPropertyManager(parent)
{
    d_ptr = new QtCustomPropertyManagerPrivate;
    d_ptr->q_ptr = this;
}

QtCustomPropertyManager::~QtCustomPropertyManager()
{
    clear();
    delete d_ptr;
}

QVariant QtCustomPropertyManager::value(const QtProperty *property) const
{
    return getValue<QVariant>(d_ptr->m_values, property);
}

void QtCustomPropertyManager::setValue(QtProperty *property, const QVariant &val)
{
    const QtCustomPropertyManagerPrivate::PropertyValueMap::iterator it =
            d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;

    QtCustomPropertyManagerPrivate::Data data = it.value();
...
void QtCustomPropertyManager::initializeProperty(QtProperty *property)
{
    d_ptr->m_values[property] = QtCustomPropertyManagerPrivate::Data();
}

void QtCustomPropertyManager::uninitializeProperty(QtProperty *property)
{
    d_ptr->m_values.remove(property);
}

bool QtCustomPropertyManager::hasCustomValue(const QtProperty */*property*/) const
{
    return true;
}

QVariant QtCustomPropertyManager::customValue(const QtProperty *property) const
{
...
public:
    ...
    void slotDeliverClickedEvent(QtProperty* property);
};

void QtButtonPropertyManagerPrivate::slotDeliverClickedEvent(QtProperty *property)
{
    emit q_ptr->buttonClicked(property);
}
...
QtButtonPropertyManager::QtButtonPropertyManager(QObject *parent)
    : QtAbstractPropertyManager(parent)
{
...
QString QtButtonPropertyManager::value(QtProperty *property) const
{
    return valueText(property);
}

void QtButtonPropertyManager::setValue(QtProperty *property, const QString &val)
{
    const QtPathPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;
