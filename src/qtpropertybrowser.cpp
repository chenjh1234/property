...
{
public:
    QtPropertyPrivate(QtAbstractPropertyManager *manager) : m_enabled(true), m_modified(false), m_expanded(true), m_visible(true), m_manager(manager) {}
    QtProperty *q_ptr;
...
bool QtProperty::isExpanded() const
{
    return d_ptr->m_expanded;
}
...
bool QtProperty::hasCustomValue() const
{
    return d_ptr->m_manager->hasCustomValue(this);
}

QVariant QtProperty::customValue() const
{
    return d_ptr->m_manager->customValue(this);
}
...
void QtProperty::setExpanded(bool expanded)
{
    if (d_ptr->m_expanded == expanded)
        return;

    d_ptr->m_expanded = expanded;
    propertyChanged();
}
...
bool QtAbstractPropertyManager::hasCustomValue(const QtProperty */*property*/) const
{
    return false;
}

QVariant QtAbstractPropertyManager::customValue(const QtProperty */*property*/) const
{
    return QVariant();
}
...
bool QtBrowserItem::hasChild() const
{
    return !d_ptr->m_children.isEmpty();
}
