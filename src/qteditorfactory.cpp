...
static void convertDoubleToSliderInt(QtDoublePropertyManager* manager, QtProperty* property, double& min, double& max, double& val)
{
    double originMin = manager->minimum(property);
    double originMax = manager->maximum(property);
    double originVal = manager->value(property);
    int decimals = manager->decimals(property);
    double factor = (decimals <= 0 ? 1 : pow(10, decimals));
    min = (factor * originMin);
    max = (factor * originMax);
    val = (factor * originVal);
}
...
// QtDoubleSliderFactoryPrivate

class QtDoubleSliderFactoryPrivate : public EditorFactoryPrivate<QSlider>
{
    QtDoubleSliderFactory *q_ptr;
    Q_DECLARE_PUBLIC(QtDoubleSliderFactory)
public:
...
void QtDoubleSliderFactoryPrivate::slotPropertyChanged(QtProperty *property, double originValue)
{
    if (!m_createdEditors.contains(property))
        return;
    ...
    int decimals = manager->decimals(property);
    double factor = (decimals <= 0 ? 1 : pow(10, decimals));
    double value = factor * originValue;
...
void QtDoubleSliderFactoryPrivate::slotRangeChanged(QtProperty *property, double min, double max)
{
    if (!m_createdEditors.contains(property))
        return;
    ...
    double value;
    convertDoubleToSliderInt(manager, property, min, max, value);
...
void QtDoubleSliderFactoryPrivate::slotSetValue(int originValue)
{
    QObject *object = q_ptr->sender();
    const QMap<QSlider *, QtProperty *>::ConstIterator ecend = m_editorToProperty.constEnd();
    for (QMap<QSlider *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor ) {
        if (itEditor.key() == object) {
            ...
            int decimals = manager->decimals(property);
            double factor = (decimals <= 0 ? 1 : pow(10, decimals));
            double value = originValue / factor;
...
// QtDoubleSliderFactory

QtDoubleSliderFactory::QtDoubleSliderFactory(QObject *parent)
    : QtAbstractEditorFactory<QtDoublePropertyManager>(parent)
{
    d_ptr = new QtDoubleSliderFactoryPrivate();
    d_ptr->q_ptr = this;
}

QtDoubleSliderFactory::~QtDoubleSliderFactory()
{
    qDeleteAll(d_ptr->m_editorToProperty.keys());
    delete d_ptr;
}

void QtDoubleSliderFactory::connectPropertyManager(QtDoublePropertyManager *manager)
{
...
QWidget *QtDoubleSliderFactory::createEditor(QtDoublePropertyManager *manager, QtProperty *property, QWidget *parent)
{
    QSlider *editor = new QSlider(Qt::Horizontal, parent);
    d_ptr->initializeEditor(property, editor);
    double min, max, val;
    convertDoubleToSliderInt(manager, property, min, max, val);
    editor->setRange(min, max);
    editor->setValue(val);
...
void QtDoubleSliderFactory::disconnectPropertyManager(QtDoublePropertyManager *manager)
{
...
void QtEnumEditorFactoryPrivate::slotPropertyChanged(QtProperty *property, int value)
{
    ...
    while (itEditor.hasNext()) {
        ...
        editor->blockSignals(true);
        editor->setCurrentIndex(manager->indexFromValue(property, value));
        editor->blockSignals(false);
    }
}
                ...
                const QStringList &enumNames)
{
    ...
    while (itEditor.hasNext()) {
        ...
        for (int i = 0; i < nameCount; i++)
            editor->setItemIcon(i, enumIcons.value(manager->valueFromIndex(property, i)));
        editor->setCurrentIndex(manager->index(property));
        editor->blockSignals(false);
    }
}
...
void QtEnumEditorFactoryPrivate::slotSetValue(int index)
{
    QObject *object = q_ptr->sender();
    ...
    for (QMap<QComboBox *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
        if (itEditor.key() == object) {
            ...
            if (!manager)
                return;
            manager->setValue(property, manager->valueFromIndex(property, index));
            return;
        }
}
...
Q_SIGNALS:
    ...
    void openTriggered(QtPopupEditorWidget*);
...
//cjh:
QString QtPopupEditorWidget::text()
{
...
public:
    ...
    void slotPropertyChanged(QtProperty *property, const QString &value);
    void slotSetValue(const QString &value);
    void slotOpenTriggered(QtPopupEditorWidget* editor);
    //cjh:
    //void slotRegExpChanged(QtProperty *property, const QRegExp &regExp);
    void slotEditingFinished();
};
...
//cjh:
void QtPopupEditorFactoryPrivate::slotEditingFinished()
{
    QObject *object = q_ptr->sender();
    //qDebug() << "is in factory";
    const EditorToPropertyMap::ConstIterator ecend = m_editorToProperty.constEnd();
    for (EditorToPropertyMap::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
    //const QMap<QLineEdit *, QtProperty *>::ConstIterator ecend = m_editorToProperty.constEnd();
    //for (QMap<QLineEdit *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
        if (itEditor.key() == object) {
            ...
            if (!manager)
                return;
            //qDebug() << "yes enter = ";
            //qDebug() << "yes enter1 = " << itEditor.key() ;
            //qDebug() << "yes enter2 = " << itEditor.key()->text() ;
            QString value = static_cast<QtPopupEditorWidget *>(itEditor.key())->text();
            //qDebug() << "yes enter v = "  << value;
...
void QtPopupEditorFactoryPrivate::slotOpenTriggered( QtPopupEditorWidget* editor )
{
    QtProperty* property = m_editorToProperty.value(editor, 0);
    if (property)
        if (QtAbstractPropertyManager* manager = property->propertyManager())
            QMetaObject::invokeMethod(manager, "slotDeliverOpenPath", Q_ARG(QtProperty*, property));
}
        ...
        QtProperty *property, QWidget *parent)
{
  ...
  //  if (regExp.isValid()) {
   ...
   //     editor->setValidator(validator);
    ...
    connect(editor, SIGNAL(destroyed(QObject *)), this, SLOT(slotEditorDestroyed(QObject *)));
    connect(editor, SIGNAL(openTriggered(QtPopupEditorWidget*)), this, SLOT(slotOpenTriggered(QtPopupEditorWidget*)));
//cjh:
    connect(editor, SIGNAL(editingFinished()),
                this, SLOT(slotEditingFinished()));
    return editor;
...
// class QtCustomEditorWidget

class QtCustomEditorWidget : public QWidget
{
    Q_OBJECT

public:
    QtCustomEditorWidget(QWidget *parent);
...
Q_SIGNALS:
    void openTriggered(QtCustomEditorWidget*);
    void popupTriggered(QtCustomEditorWidget*);

private:
    QToolButton *m_buttonOpen;
    QToolButton *m_buttonPopup;
    QVariant customBackground;
};

QtCustomEditorWidget::QtCustomEditorWidget(QWidget *parent)
    : QWidget(parent)
{
    QHBoxLayout *layout = new QHBoxLayout(this);
    setupTreeViewEditorMargin(layout);
    ...
    m_buttonOpen->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Ignored);
    m_buttonOpen->setFixedWidth(20);
    setFocusProxy(m_buttonOpen);
    setFocusPolicy(m_buttonOpen->focusPolicy());
    m_buttonOpen->setText(tr("..."));
    m_buttonOpen->installEventFilter(this);
    connect(m_buttonOpen, SIGNAL(clicked()), this, SLOT(buttonClicked()));

    m_buttonPopup = new QToolButton;
    m_buttonPopup->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Ignored);
    m_buttonPopup->setFixedWidth(20);
    m_buttonPopup->setArrowType(Qt::DownArrow);
    setFocusProxy(m_buttonPopup);
    setFocusPolicy(m_buttonPopup->focusPolicy());
//    m_buttonPopup->setText(tr("..."));
    m_buttonPopup->installEventFilter(this);
    connect(m_buttonPopup, SIGNAL(clicked()), this, SLOT(buttonPopup()));

    layout->addWidget(m_buttonOpen);
    layout->addWidget(m_buttonPopup);
}

void QtCustomEditorWidget::buttonClicked()
{
    emit openTriggered(this);
}

void QtCustomEditorWidget::buttonPopup()
{
    emit popupTriggered(this);
}

void QtCustomEditorWidget::mouseDoubleClickEvent(QMouseEvent */*event*/)
{
    emit openTriggered(this);
}

void QtCustomEditorWidget::setValue(const QVariant &value)
{
    customBackground = value;
    this->update();
}

bool QtCustomEditorWidget::eventFilter(QObject *obj, QEvent *ev)
{
    if (obj == m_buttonOpen || obj == m_buttonPopup) {
        switch (ev->type()) {
        case QEvent::KeyPress:
        case QEvent::KeyRelease: {
...
void QtCustomEditorWidget::paintEvent(QPaintEvent *e)
{
    QPainter painter(this);
    if (customBackground.canConvert(QVariant::Image)) {
        QImage image = customBackground.value<QImage>();
        if (!image.isNull()) {
            painter.drawImage(e->rect(), image);
        }
    }
    else if (customBackground.canConvert(QVariant::Pixmap)) {
        QPixmap pixmap = customBackground.value<QPixmap>();
        if (!pixmap.isNull()) {
            painter.drawPixmap(e->rect(), pixmap);
        }
    }
    else if (customBackground.canConvert(QVariant::Color)
             || customBackground.canConvert<QRgb>())
    {
        QColor color = (customBackground.canConvert(QVariant::Color) ?
                            customBackground.value<QColor>() :
                            QColor(customBackground.value<QRgb>()));
        painter.fillRect(e->rect(), color);
    }
    /*
    else if (customBackground.canConvert<EsColors>()) {
        EsColors colors = customBackground.value<EsColors>();
        QLinearGradient gradient = colors.toGradient();
        gradient.setStart(e->rect().left(), 0);
        gradient.setFinalStop(e->rect().right(), 0);
        painter.fillRect(e->rect(), QBrush(gradient));
    }*/
}



// QtCustomEditorFactory

class QtCustomEditorFactoryPrivate : public EditorFactoryPrivate<QtCustomEditorWidget>
{
    QtCustomEditorFactory *q_ptr;
    Q_DECLARE_PUBLIC(QtCustomEditorFactory)
public:
    void slotOpenTriggered(QtCustomEditorWidget* editor);
    void slotPopupTriggered(QtCustomEditorWidget* editor);
    void slotPropertyChanged(QtProperty *property, const QVariant &value);
};

void QtCustomEditorFactoryPrivate::slotOpenTriggered(QtCustomEditorWidget *editor)
{
    QtProperty* property = m_editorToProperty.value(editor, 0);
    if (property)
        if (QtAbstractPropertyManager* manager = property->propertyManager())
        {
            QMetaObject::invokeMethod(manager,
                                      "slotDeliverOpenEvent", Q_ARG(QtProperty*, property));
        }
}

void QtCustomEditorFactoryPrivate::slotPopupTriggered(QtCustomEditorWidget *editor)
{
    QtProperty* property = m_editorToProperty.value(editor, 0);
    if (property)
        if (QtAbstractPropertyManager* manager = property->propertyManager())
        {
            QMetaObject::invokeMethod(manager,
                                      "slotDeliverPopupEvent", Q_ARG(QtProperty*, property));
        }
}

void QtCustomEditorFactoryPrivate::slotPropertyChanged(QtProperty *property, const QVariant &value)
{
    const PropertyToEditorListMap::iterator it = m_createdEditors.find(property);
    if (it == m_createdEditors.end())
        return;
    QListIterator<QtCustomEditorWidget *> itEditor(it.value());
...
QtCustomEditorFactory::QtCustomEditorFactory(QObject *parent)
    : QtAbstractEditorFactory<QtCustomPropertyManager>(parent),
    d_ptr(new QtCustomEditorFactoryPrivate())
{
    d_ptr->q_ptr = this;
}

QtCustomEditorFactory::~QtCustomEditorFactory()
{
    qDeleteAll(d_ptr->m_editorToProperty.keys());
    delete d_ptr;
}

void QtCustomEditorFactory::connectPropertyManager(QtCustomPropertyManager *manager)
{
    connect(manager, SIGNAL(valueChanged(QtProperty *, const QVariant &)), this, SLOT(slotPropertyChanged(QtProperty*,const QVariant &)));
}

QWidget *QtCustomEditorFactory::createEditor(QtCustomPropertyManager *manager, QtProperty *property, QWidget *parent)
{
    QtCustomEditorWidget *editor = d_ptr->createEditor(property, parent);
    editor->setValue(manager->value(property));
    connect(editor, SIGNAL(destroyed(QObject *)), this, SLOT(slotEditorDestroyed(QObject *)));
    connect(editor, SIGNAL(openTriggered(QtCustomEditorWidget*)), this, SLOT(slotOpenTriggered(QtCustomEditorWidget*)));
    connect(editor, SIGNAL(popupTriggered(QtCustomEditorWidget*)), this, SLOT(slotPopupTriggered(QtCustomEditorWidget*)));
    return editor;
}

void QtCustomEditorFactory::disconnectPropertyManager(QtCustomPropertyManager *manager)
{
    disconnect(manager, SIGNAL(valueChanged(QtProperty *, const QVariant &)), this, SLOT(slotPropertyChanged(QtProperty*,const QVariant &)));
}


// QtButtonWidget

class QtButtonWidget : public QWidget
{
    Q_OBJECT
public:
    QtButtonWidget(QWidget *parent);
...
Q_SIGNALS:
    void valueChanged(const QString &value);
    void buttonClicked(QtButtonWidget*);

private:
    QToolButton *m_button;
};

QtButtonWidget::QtButtonWidget(QWidget *parent)
    : QWidget(parent)
{
    QHBoxLayout *layout = new QHBoxLayout(this);
    setupTreeViewEditorMargin(layout);
...
void QtButtonWidget::setValue(const QString &value)
{
    m_button->setText(value);
}

void QtButtonWidget::onButtonClicked()
{
    emit buttonClicked(this);
}

bool QtButtonWidget::eventFilter(QObject *obj, QEvent *ev)
{
    if (obj == m_button) {
        switch (ev->type()) {
        case QEvent::KeyPress:
...
// QtButtonFactoryPrivate

class QtButtonFactoryPrivate : public EditorFactoryPrivate<QtButtonWidget>
{
...
public:
    void slotPropertyChanged(QtProperty *property, const QString &value);
    void slotSetValue(const QString &value);
    void slotButtonClicked(class QtButtonWidget* editor);
};

void QtButtonFactoryPrivate::slotPropertyChanged(QtProperty *property, const QString &value)
{
    const PropertyToEditorListMap::iterator it = m_createdEditors.find(property);
    if (it == m_createdEditors.end())
        return;
    QListIterator<QtButtonWidget*> itEditor(it.value());

    while (itEditor.hasNext())
        itEditor.next()->setValue(value);
}

void QtButtonFactoryPrivate::slotSetValue(const QString &value)
{
    QObject *object = q_ptr->sender();
    const EditorToPropertyMap::ConstIterator ecend = m_editorToProperty.constEnd();
    for (EditorToPropertyMap::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
...
void QtButtonFactoryPrivate::slotButtonClicked(QtButtonWidget *editor)
{
    QtProperty* property = m_editorToProperty.value(editor, 0);
    if (property)
        if (QtAbstractPropertyManager* manager = property->propertyManager())
            QMetaObject::invokeMethod(manager, "slotDeliverClickedEvent", Q_ARG(QtProperty*, property));
}
...
QWidget *QtButtonFactory::createEditor(QtButtonPropertyManager *manager, QtProperty *property, QWidget *parent)
{
    QtButtonWidget *editor = d_ptr->createEditor(property, parent);
    editor->setValue(manager->value(property));
    connect(editor, SIGNAL(valueChanged(const QString &)), this, SLOT(slotSetValue(const QString &)));
    connect(editor, SIGNAL(destroyed(QObject *)), this, SLOT(slotEditorDestroyed(QObject *)));
    connect(editor, SIGNAL(buttonClicked(QtButtonWidget*)),  this, SLOT(slotButtonClicked(QtButtonWidget*)));
    return editor;
}
