...
enum { CustomDisplayRole = Qt::UserRole + 1 };
...
void QtPropertyEditorView::drawRow(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (!hasValue && m_editorPrivate->markPropertiesWithoutValue() && !index.parent().isValid()) {
            ...
            const QModelIndex &index) const
{
    ...
    // custom drawing background of item
    bool hasDrawnBackground = false;
    QVariant customBackground = index.data(CustomDisplayRole);
   if (customBackground.isValid())
   {
       if (customBackground.canConvert(QVariant::Icon)) {
           ; // do nothing
       }
       else if (customBackground.canConvert(QVariant::Image)) {
       ...
       /*
            ...
            gradient.setStart(option.rect.left(), 0);
            gradient.setFinalStop(option.rect.right(), 0);
...
QIcon QtPropertyEditorView::drawIndicatorIcon()
{
    QPixmap pix(14, 14);
    ...
    QStyleOptionViewItemV2 branchOption = this->viewOptions();
...
//    labels.append(QApplication::translate("QtTreePropertyBrowser", "Value" , 0, QApplication::UnicodeUTF8));
 ...
 //   m_treeWidget->header()->setMovable(false);
  //  m_treeWidget->header()->setResizeMode(QHeaderView::Stretch);
    ...
    m_expandIcon = m_treeWidget->drawIndicatorIcon();
...
void QtTreePropertyBrowserPrivate::propertyInserted(QtBrowserItem *index, QtBrowserItem *afterIndex)
{
    ...
    m_treeWidget->setItemExpanded(newItem, index->property()->isExpanded());
...
void QtTreePropertyBrowserPrivate::updateItem(QTreeWidgetItem *item)
{
    ...
    if (property->hasValue()) {
        ...
        if (property->hasCustomValue()) {
            item->setData(1, CustomDisplayRole, property->customValue());
        }
        else {
...
//  bool wasEnabled = item->flags() & Qt::ItemIsEnabled;
    bool wasEnabled = item->flags() & Qt::ItemIsEditable;
    bool isEnabled = wasEnabled;
    if (property->isEnabled()) {
        QTreeWidgetItem *parent = item->parent();
//      if (!parent || (parent->flags() & Qt::ItemIsEnabled))
        if (!parent || (parent->flags() & Qt::ItemIsEditable))
            isEnabled = true;
        else
            isEnabled = false;
    } else {
...
void QtTreePropertyBrowser::setPropertiesWithoutValueMarked(bool mark)
{
    ...
    while (it.hasNext()) {
        QtProperty *property = it.next().value()->property();
        if (!property->hasValue() && !it.value()->parent())
            d_ptr->updateItem(it.key());
    }
    d_ptr->m_treeWidget->viewport()->update();
}
